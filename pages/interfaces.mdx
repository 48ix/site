...
description: Understand how to configure your network device when connecting to the exchange.
...
48 IX uses [MAC address](https://en.wikipedia.org/wiki/MAC_address) filtering and persistent MAC learning (also commonly referred to as 'sticky MAC') to limit the MAC addresses allowed to communicate on a port to the first **two** learned when a port first comes online. The [48 IX Connection Agreement](connection-agreement.mdx) allows for connecting up to **two** routers to each member's port.
