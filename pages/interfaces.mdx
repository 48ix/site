...
48-IX uses [MAC address](https://en.wikipedia.org/wiki/MAC_address) filtering and persistent MAC learning (also commonly referred to as 'sticky MAC') to limit the MAC addresses allowed to communicate on a port to the first one learned when a port first comes online. The 48-IX Connection Agreement allows for connecting one router to each member's port.
