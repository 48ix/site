...
type Animation = { letter: MotionProps; line: MotionProps };

export const Logo: React.FC<LogoProps> = (props: LogoProps) => {
  const { size = 128, strokeWidth = 10, noanimate = false, ...rest } = props;
  const id = useUniqueId('logo_');

  const animation = useMemo<Animation>((): Animation => {
    if (noanimate) {
      return {
        line: {},
        letter: {},
  ...
  const ixColor = useColorToken('gray.800', 'red.400');
