...
const StateOutline = ({ size = 128, strokeWidth = 10, noanimate = false, ...props }) => {
  const ComponentSvg = { true: 'svg', false: motion.svg };
  const ComponentPath = { true: 'path', false: motion.path };
  const Svg = ComponentSvg[noanimate];
  const Path = ComponentPath[noanimate];
...
StateOutline.displayName = 'StateOutline';
